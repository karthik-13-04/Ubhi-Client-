...
(function () {
  ...
  var SETTINGS_KEY = 'ubhi-site-settings';
  var usedKeys = {};
  var editMode = false;
  var imageMode = false;
  var built = false;
  ...
  function setSettings(o) {
    ...
    if (window.ubhiSyncPush) { try { window.ubhiSyncPush('site-settings', o); } catch (e) {} }
  }
  ...
  function injectStyles() {
    var css =
      ...
      '.ubhi-im-on [data-em-image-key]:hover{outline:2px dashed #9e7c3e;outline-offset:4px;cursor:pointer;}' +
      '.ubhi-im-on [data-em-image-key]{transition:outline-color .2s ease, transform .2s ease;}' +
      '.ubhi-im-active{outline:2px solid #b68a3e !important;outline-offset:4px;transform:scale(1.01);}' +
  ...
  function setImageMode(on) {
    imageMode = on;
    document.body.classList.toggle('ubhi-im-on', on);
    if (!on) {
      var active = document.querySelector('[data-em-image-key].ubhi-im-active');
      if (active) active.classList.remove('ubhi-im-active');
    } else {
      collectImages();
    }
  }
  ...
  function imageKeyFor(el, index) {
    if (el.dataset.emImageKey) return el.dataset.emImageKey;
    var page = (el.closest('[id^="page-"]') || {}).id || 'site';
    var src = el.getAttribute('src') || '';
    var key = page + '|img|' + index + '|' + hash(src);
    el.dataset.emImageKey = key;
    return key;
  }
  function collectImages() {
    ...
    document.querySelectorAll('img').forEach(function (el) {
      if (!el || el.closest('#page-admin, #page-account, .modal, .lightbox, .admin-tab-content, [data-noedit]')) return;
      if (el.closest('.product-card, .workshop-card, .journal-card, .gallery-item, .artfolio-card, .preview-card, .member-card')) return;
      if (!el.getAttribute('src')) return;
      imageKeyFor(el, index++);
      list.push(el);
    });
    return list;
  }
  function applyImageOverrides() {
    ...
    var overrides = settings.mediaOverrides || {};
    collectImages().forEach(function (el) {
      var key = el.dataset.emImageKey;
      if (key && overrides[key] && el.getAttribute('src') !== overrides[key]) {
        el.setAttribute('src', overrides[key]);
      }
    });
  }
  function saveImageOverride(key, src) {
    var settings = getSettings();
    settings.mediaOverrides = settings.mediaOverrides || {};
    if (src) settings.mediaOverrides[key] = src;
    else delete settings.mediaOverrides[key];
    setSettings(settings);
    applyImageOverrides();
    toast('Image saved');
  }
  function canUploadImages() {
    return typeof window.ubhiUpload === 'function' && typeof window.ubhiSyncStatus === 'function' &&
      window.ubhiSyncStatus().connected;
  }
  function pickImageSource(el) {
    var pasted = window.prompt('Paste an image URL here.\nLeave it blank and press OK to upload a file instead.', el.getAttribute('src') || '');
    if (pasted === null) return;
    pasted = String(pasted || '').trim();
    if (pasted) {
      saveImageOverride(el.dataset.emImageKey, pasted);
      return;
    }
    if (!canUploadImages()) {
      window.alert('Connect the admin panel to the server first before uploading images.');
      return;
    }
    ...
    input.addEventListener('change', function () {
      ...
      reader.onload = function () {
        Promise.resolve(window.ubhiUpload(reader.result)).then(function (url) {
          if (!url) {
            window.alert('Image upload failed. Please confirm the server connection and try again.');
            return;
          }
          saveImageOverride(el.dataset.emImageKey, url);
        });
      };
  ...
  function routeFor(pathname) {
    return (pathname || '/') + (pageRoutes[pathname || '/'] || '');
  }
  function launchPageEditor(mode) {
    var select = document.getElementById('admin-profile-page-select');
    var route = select ? select.value : '/';
    try {
      sessionStorage.setItem('ubhi-profile-action', JSON.stringify({ route: route, mode: mode }));
    } catch (e) {}
    window.location.href = routeFor(route);
  }
  function updateProfileSummary() {
    var box = document.getElementById('admin-profile-page-summary');
    var select = document.getElementById('admin-profile-page-select');
    if (!box || !select) return;
    var route = select.value || '/';
    var name = select.options[select.selectedIndex] ? select.options[select.selectedIndex].textContent : route;
    box.innerHTML = '<strong>' + name + '</strong><br>Use <em>Edit text</em> to change headings and copy, or <em>Edit images</em> to replace page images live on the selected route.';
  }
  function initProfileActions() {
    var textBtn = document.getElementById('admin-profile-edit-text');
    var imageBtn = document.getElementById('admin-profile-edit-images');
    var refreshBtn = document.getElementById('admin-profile-refresh');
    var select = document.getElementById('admin-profile-page-select');
    var launch = document.querySelector('.ubhi-em-launch');
    if (launch && !launch._profileRedirect) {
      launch._profileRedirect = true;
      launch.addEventListener('click', function () { launchPageEditor('text'); });
    }
    if (textBtn && !textBtn._wired) {
      textBtn._wired = true;
      textBtn.addEventListener('click', function () { launchPageEditor('text'); });
    }
    if (imageBtn && !imageBtn._wired) {
      imageBtn._wired = true;
      imageBtn.addEventListener('click', function () { launchPageEditor('image'); });
    }
    if (refreshBtn && !refreshBtn._wired) {
      ...
      refreshBtn.addEventListener('click', function () {
        ...
        if (msg) msg.textContent = 'Content reloaded from the latest saved state.';
      });
    }
    if (select && !select._wired) {
      select._wired = true;
      select.addEventListener('change', updateProfileSummary);
    }
    updateProfileSummary();
  }
  function resumeQueuedAction() {
    var raw;
    try { raw = sessionStorage.getItem('ubhi-profile-action'); } catch (e) {}
    if (!raw) return;
    try {
      var queued = JSON.parse(raw);
      if (!queued || queued.route !== (window.location.pathname || '/')) return;
      sessionStorage.removeItem('ubhi-profile-action');
      setTimeout(function () {
        if (queued.mode === 'image') setImageMode(true);
  ...
  document.addEventListener('click', function (e) {
    if (imageMode) {
      if (e.target.closest('.ubhi-em-ui')) return;
      var img = e.target.closest('img[data-em-image-key]');
      if (img) {
        e.preventDefault();
        e.stopPropagation();
        document.querySelectorAll('.ubhi-im-active').forEach(function (node) { node.classList.remove('ubhi-im-active'); });
        img.classList.add('ubhi-im-active');
        pickImageSource(img);
        return;
      }
    }
  ...
  window.ubhiEditMode = {
    enable: function () { if (isOwner()) { buildUI(); setImageMode(false); setEditMode(true); } },
    disable: function () { setEditMode(false); setImageMode(false); },
    enableImages: function () { if (isOwner()) { buildUI(); setEditMode(false); setImageMode(true); } },
    refresh: refresh
  };
})();
